...
ThirdPersonCamera.prototype.update = function(positioning, head, direction) {
    //////////////////////////////////////////////////////////
    ...
    // NOTE:
    // For every popped interval in the array, the number of
    // cubes remaining for the camera to transition should
    // never be 0, until it decrements to 0, then it no
    // longer remains in the array.

    // Get the array of intervalPositions, which is held
    // by the parameter positioning
    // Update the camera's current Position
    // pop the first element in the positioning array by a call to shift()
    // to get an intervalPosition object.
    var interval = positioning.shift();
    ...
    // What if a wall interferes with the camera's trajectory, then do the following:

    // Get the steps from the camera's current position to the wall's entry, or the cube
    // position right before the wall, note that the cube that collided with the wall had
    // its position reset directly from the opposing wall. So for detecting head collision if
    // true, then the wallEntrance is the cube before the head (snake.length - 2).
    
    // The number of cubes that are between the cube position the camera is currently relative to
    // and the wall where collision is detected.
    ...
    // Sacrifice a frame of rotation to create this jump.
